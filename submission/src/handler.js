...
const createBooks = (request, h) => {
    ...
    const bookIndex = bookshelf.filter((book) => book.id === id).length > 0;
...
const getBooks = (request, h) => {
    ...
    if (name) {
      index = bookshelf.filter((book) =>
        book.name.toLowerCase().includes(name.toLowerCase())
      );
    }
  
    if (reading) {
      index = bookshelf.filter(
...
module.exports = {createBooks, getBooks, getBooksId, editBooksId, deleteBooksId};